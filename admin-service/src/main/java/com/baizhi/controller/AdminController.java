...
public class AdminController {
    ...
    public Map<String, String> captcha() {
        ...
        // 将验证码保存到redis中  加前缀是为了分辨是验证码的key
        stringRedisTemplate.opsForValue().set(RedisPrefix.CAPTCHA_CODE + token, value, 5, TimeUnit.MINUTES);
        // 生成base64
    ...
    public Integer adminStatus(String token){
        Boolean b=stringRedisTemplate.hasKey(RedisPrefix.USER_LOGIN +token);
        if(b==true){
            return 1;
        }else {
            return 2;
